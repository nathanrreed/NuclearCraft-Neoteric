...
public class NCLootTables extends BaseLootTableProvider {
    public NCLootTables(HolderLookup.Provider provider) {
        super(provider);
    }
    ...
    private void blocks() {
        for (DeferredBlock<Block> block : NC_BLOCKS.values()) {
            add(block);
        }
        for (DeferredBlock<Block> block : NC_MATERIAL_BLOCKS.values()) {
            add(block);
        }

        KUGELBLITZ_BLOCKS.values().forEach(this::add);
        FISSION_BLOCKS.values().forEach(this::add);
        for (DeferredBlock<Block> block : FUSION_BLOCKS.values()) {
            add(block);
        }
        for (DeferredBlock<Block> block : TURBINE_BLOCKS.values()) {
            add(block);
        }
    }

    private void add(DeferredBlock<Block> regBlock) {
    ...
    protected @NotNull Iterable<Block> getKnownBlocks() {
        ...
        all.addAll(ORE_BLOCKS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(NC_BLOCKS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(NC_MATERIAL_BLOCKS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(FISSION_BLOCKS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(FUSION_BLOCKS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(TURBINE_BLOCKS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(KUGELBLITZ_BLOCKS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(PROCESSORS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(ENERGY_BLOCKS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(STORAGE_BLOCKS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(NC_ELECTROMAGNETS.values().stream().map(DeferredBlock::get).toList());
        all.addAll(NC_RF_AMPLIFIERS.values().stream().map(DeferredBlock::get).toList());
