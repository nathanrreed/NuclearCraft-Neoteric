...
public class ItemStackIngredientCreator implements IItemStackIngredientCreator {
    ...
    public static String getName(Ingredient ingredient) {
        if (ingredient.getValues()[0] instanceof Ingredient.TagValue(TagKey<Item> tag)) {
            return (tag.location().getPath().replace("/", "_"));
        } else if (ingredient.getValues()[0] instanceof Ingredient.ItemValue(ItemStack itemStack)) {
            return ResourceLocation.parse(itemStack.getItem().toString()).getPath().replace("/", "_");
        } else {
            return "ERROR!";
        }
    }
    ...
    public ItemStackIngredient from(List<Ingredient> ingredients, int amount) {
        if (ingredients.size() == 1) {
            return new SingleItemStackIngredient(ingredients.getFirst(), amount);
        }
        return new MultiItemStackIngredient(ingredients, amount);
    }
    ...
    public static class SingleItemStackIngredient extends ItemStackIngredient {
        ...
        public List<Ingredient> getInputsRaw() {
        ...
        public String getName() {
            return ItemStackIngredientCreator.getName(ingredient);
        }
    ...
    public static class MultiItemStackIngredient extends ItemStackIngredient implements IMultiIngredient<ItemStack, ItemStackIngredient> {
        ...
        public MultiItemStackIngredient(List<Ingredient> ingredients, Integer amount) {
            this.ingredients = ((MultiItemStackIngredient) ItemStackIngredientCreator.INSTANCE.createMulti(ingredients.stream().map(i -> new SingleItemStackIngredient(i, amount)).toArray(ItemStackIngredient[]::new))).ingredients;
        }
        ...
        @Override
        public String getName() {
            return ItemStackIngredientCreator.getName(getInputsRaw().getFirst());
        }

        @Override
        public int getAmount() {
            return getRepresentations().getFirst().getCount();
        }
        ...
        public List<Ingredient> getInputsRaw() {
            return Arrays.stream(ingredients).map(single -> ((SingleItemStackIngredient) single).ingredient).toList();
        }
